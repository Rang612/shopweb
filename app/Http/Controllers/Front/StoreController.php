...
//    }
    ...
    public function index(Request $request)
    {
        ...
        foreach ($stores as $store) {
            if ($store->image_url && file_exists(public_path('../shared_uploads/' . ltrim($store->image_url, '/')))) {
                // Ảnh local từ thư mục chia sẻ
                $store->saved_image = $store->image_url;
            } else {
                $store->saved_image = asset('front/img/default-store.jpg');
            }
        }
