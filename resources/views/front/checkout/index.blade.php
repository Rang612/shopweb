...
{{--        try {--}}
{{--            let response = await fetch("/proxy/provinces");--}}
...
{{--                citySelect.innerHTML += `<option value="${province.code}">${cleanName}</option>`; // Dùng code thay vì cleanName--}}
{{--            });--}}
{{--        } catch (error) {--}}
...
{{--        try {--}}
{{--            const response = await fetch(`/proxy/provinces/${cityCode}`);--}}
...
{{--        try {--}}
{{--            const response = await fetch(`/proxy/districts/${districtCode}`);--}}
...
{{--            let cityName = this.options[this.selectedIndex].text; // Lấy tên tỉnh để tính phí ship--}}
{{--            let token = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content');--}}

{{--            if (cityCode) {--}}
{{--                loadDistricts(cityCode); // Load quận/huyện dựa trên cityCode--}}
{{--            }--}}
...
{{--                //         'X-CSRF-TOKEN': token--}}
{{--                //     },--}}
{{--                //     body: JSON.stringify({ city: cityName }) // Gửi tên tỉnh để tính phí ship--}}
{{--                // })--}}
...
{{--                    .then(data => {--}}
{{--                        let shippingCost = parseFloat(data.shipping_cost);--}}
{{--                        document.getElementById('shipping_cost').value = shippingCost; // Cập nhật giá trị shipping_cost--}}
{{--                        let shippingCostElement = document.querySelector('.shipping-cost span');--}}
{{--                        let previousShippingCost = parseFloat(shippingCostElement.textContent.replace(/[^\d]/g, '')) || 0;--}}
{{--                        shippingCostElement.textContent = shippingCost.toLocaleString('vi-VN') + " VND";--}}
 ...
 <script>
     ...
     async function loadDistricts(cityName) {
         try {
             ...
             const provinceResponse = await fetch("/proxy/provinces");
             ...
             const response = await fetch(`/proxy/provinces/${province.code}`);
     ...
     async function loadWards(districtName) {
         try {
             ...
             const districtResponse = await fetch(`/proxy/provinces/${province.code}`);
             ...
             const response = await fetch(`/proxy/districts/${district.code}`);
